...
/// @notice Orchestrates all operator operations across yield strategies and vaults
/// @dev Central control point for AI agent to manage allocations and harvesting
contract YieldOrchestrator is AccessControl, ReentrancyGuard {
    ...
    // Strategy registry per asset: asset => strategy
    mapping(address => address) public strategyOf;

    // Track all strategies for scheduled harvesting
    address[] public strategies;
    mapping(address => bool) public isStrategyRegistered;

    // Configuration
    uint256 public constant SLIPPAGE_TOLERANCE = 9500; // 95% (5% slippage) - informative default
    ...
    constructor(address _admin, address _operator, address _uniswapRouter, uint24 _defaultPoolFee) {
    ...
    /// @notice Register a yield strategy for a given asset
    ...
    function setStrategy(address asset, address strategy) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        require(IYieldStrategy(strategy).asset() == asset, "mismatch");
    ...
    function harvestAll() external onlyRole(OPERATOR_ROLE) {
        uint256 len = strategies.length;
        for (uint256 i = 0; i < len; i++) {
            _harvestStrategy(strategies[i]);
        }
    }

    /// @notice Harvest yield from a specific strategy
    /// @param strategy Strategy address
    function harvestStrategy(address strategy) external onlyRole(OPERATOR_ROLE) {
    ...
    function _harvestStrategy(address strategy) internal {
        ...
        (uint256 profit, uint256 loss) = IYieldStrategy(strategy).report();
        emit YieldHarvested(strategy, profit, loss);
    }

    /// @notice Deposit ERC20 on behalf of a user, optionally swap to target asset, then deposit into strategy
    ...
    function depositERC20(
        ...
        address receiver
    ) external onlyRole(OPERATOR_ROLE) nonReentrant returns (uint256 sharesOut) {
        require(from != address(0) && receiver != address(0), "bad addr");
        require(amountIn > 0, "zero amount");
        address strategy = strategyOf[targetAsset];
        ...
        sharesOut = IYieldStrategy(strategy).deposit(amountToDeposit, receiver);

        emit Deposited(from, inputAsset, targetAsset, amountIn, sharesOut, receiver);
    }

    /// @notice Withdraw from a strategy on behalf of a user to a desired asset (optional swap), transfer to receiver
    ...
    function withdrawERC20(
        ...
        emit Withdrawn(owner, strategyAsset, outputAsset, assets, amountOut, receiver);
    }

    /// @notice Reallocate a user's position across assets
    ...
    function reallocate(
        ...
        address sourceStrategy = strategyOf[sourceAsset];
        address targetStrategy = strategyOf[targetAsset];
        ...
        IYieldStrategy(targetStrategy).deposit(amountToDeposit, owner);

        emit Reallocated(owner, sourceAsset, targetAsset, shares, amountToDeposit);
    }
