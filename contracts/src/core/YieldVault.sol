...
/// @title YieldVault
/// @notice ERC-4626 vault that supplies assets to Aave V3 with performance fees and yield distribution
contract YieldVault is ERC4626, AccessControl, Pausable, ReentrancyGuard {
    ...
    // --- Events ---
    event SuppliedToAave(address indexed asset, uint256 amount, address indexed operator);
    event WithdrawnFromAave(address indexed asset, uint256 amount, address indexed operator);
    ...
    constructor(
        ...
        ERC20(
            string.concat("Yield ", IERC20Metadata(address(asset_)).symbol(), " Vault"),
            string.concat("yV-", IERC20Metadata(address(asset_)).symbol())
        )
        ERC4626(asset_)
    {
        ...
        // Default to public deposits enabled
    ...
    function supplyToAave(uint256 amount) external whenNotPaused onlyRole(OPERATOR_ROLE) {
        if (amount == 0) return;
    ...
    function withdrawFromAave(uint256 amount) external whenNotPaused onlyRole(OPERATOR_ROLE) {
        if (amount == 0) return;
    ...
    function takeFees() public whenNotPaused onlyRole(OPERATOR_ROLE) {
    ...
    function checkpoint() public whenNotPaused onlyRole(OPERATOR_ROLE) {
