...
const YIELD_VAULT_ABI = [
  ...
  { name: "supplyToAave", type: "function", stateMutability: "nonpayable", inputs: [{ name: "amount", type: "uint256" }], outputs: [] },
] as const;
...
export async function POST() {
  try {
    ...
    const idle = await publicClient.readContract({ address: vaultAddress, abi: YIELD_VAULT_ABI, functionName: "idleUnderlying" });
    if (idle === BigInt(0)) return NextResponse.json({ ok: true, skipped: true, reason: "no idle funds", amount: "0" });
    ...
    return NextResponse.json({ ok: true, tx_hash: hash, amount: idle.toString() });
  } catch (e: any) {
